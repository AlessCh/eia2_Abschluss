namespace footballSimulator {
    ...
    const canvasGround = document.getElementById("playground") as HTMLCanvasElement;
    ...
    const startGameButton = document.getElementById("starter") as HTMLButtonElement;
    ...
    export interface change { jersy: number; team: string; }
    ...
    function setupTeams(
        teamOneColor: string = "#00ffaa", //default
        teamTwoColor: string = "#d703fc", //default
        ...
        setupTeam(
            "Team One",
            standardPositionsTeamOne, //coordinates from array for team one
            teamOneColor,
        ...
        setupTeam(
            "Team Two",
            standardPositionsTeamTwo, //coordinates from array for team two
            teamTwoColor,
    ...
    function setupTeam(
        ...
        for (let i = 1; i <= 11; i++) {
            const player = new Player(
                positions[i - 1], //parameter positions: coordinate
                ...
                i, //let i = number
                false,
                jerseyColor, //parameter jerseycolor = string
        ...
        players.forEach((player) => { ///performs specified action for each element in an array
    ...
    function insideGoal(coordinate: Coordinate, borders: Coordinate[]): boolean {
        ...
        borders.forEach((coord) => { //arrow function - function has only one statement, and the statement returns a value, shorter than writing it out // no binding of this
            xBoarders.add(coord.x);
            yBoarders.add(coord.y);
        });

        const xValueArr: number[] = Array.from(xBoarders) as number[]; //Array.from creates array from iterable object -> array ähnliches object converted to array
        xValueArr.sort((a: number, b: number) => a - b); //sort = sorts an array in place, mutates array and returns reference to same array
    ...
    function kickBall(event: MouseEvent): void {
        ...
        for (let i = 0; i < movable.length; i++) {
            if (movable[i] instanceof Ball) { //expected output truw
